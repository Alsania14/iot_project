...
/* SDA LCD A4, SCL A5
 *  D12 -> GPIO2 esp
 *  D3 -> GPIO0 (INGET KASI VOLTAGE DIVIDER, BIAR 5V NYA ARDUINO JADI 3.3v LOGIC)
 *  D2 -> servo
...
void loop() {
  ...
  if (buttonState == HIGH) {
    ...
    delay(3000); // delay 3 detik an, tujuannya biar esp nya bisa baca, biar ga langsung di pull low lagi
  ...
  if(start == 1 || espGateState != espGateStateLast) { // bandingin statenya, klo ada perubahan state baru masuk sini
    ...
    if (espGateState == HIGH ) {
      servo.write(angle2); //open
      writeLCD(1);
    }
    else if (espGateState == LOW) {
      servo.write(angle); //close
      writeLCD(0);
    }
